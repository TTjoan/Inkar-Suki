...
async def _(event: Union[GroupMessageEvent, PrivateMessageEvent]):
    ...
    await hid_uid.finish(f'已{"关闭" if raw_stats else "开启"}uid遮挡', at_sender=True)
...
async def _():
    ...
    for file, url in resource_pool.items():
        await update_resource.send(f"正在更新：{file}")
...
async def _(args: Message = CommandArg()):
    ...
    if len(args) > 1:
        ...
        await add_aliases.finish(f"别称添加完成：{identify_name}({hash_id})：{aliases}")
...
async def _(bot: Bot, event: Union[GroupMessageEvent, PrivateMessageEvent]):
    ...
    for lang, lang_data in file_pool["loc.json"].items():
        ...
        for hash_id, aliases_list in Data(Data.globals, "genshin_game_data").get_data(key="character_aliases", default={}).items():
            ...
            if os.path.exists(
                os.path.join(
                    Path.res, f"textures/genshin/{character_name_input}.png"
                )
            ):
    ...
    await character_img.finish(
        MessageSegment.image(
            file=f'file:///{os.path.join(Path.res, f"textures/genshin/{chinese_name}.png")}'
        )
    )
